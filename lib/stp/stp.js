...
(function(root) {
    ...
    _stp.Node = function(name, duration) {
        this.name = name;
    ...
    _stp.STP.prototype.getEdgesOfNode = function(node, both_direction) {
        var edge_list = [];
        for(var k in this.e) {
            var edge = this.e[k];
            if((edge.vi === node && (both_direction || !both_direction && edge.wij >= 0)) 
                || (edge.vj === node && (both_direction || !both_direction && edge.wji > 0))) {
                ...
                while ((n_list_intersection = intersection(v, stp.v)).length > 0) {
                    ...
                    for ( var ki in neighbors_list) {
                        ...
                        for ( var kj in neighbors_list) {
                            ...
                            t_stack.push(new _stp.Triangle(vi, vj, vk));
                        }
                    }
                }
                return t_stack;
