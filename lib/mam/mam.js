...
(function(root) {
    var _mam = function(name, mailbox) {
        this.name = name;
    ...
    _mam.prototype.handleMessage = function(msg) {
        if(msg.to == this.name) {
            ...
            if(a === undefined) {
                a = abt.createSlave(this.name, msg.subject, this.getAvailableHours(0, 100), msg.from, this.mailbox);
    ...
    _mam.prototype.createSharedAppointment = function(desc, start, end, duration, invitee_list) {
        ...
        this.shared_appointment_list.push(abt.createMaster(this.name, desc, range, invitee_list, this.mailbox));
    },
    _mam.prototype.addAppointment = function(desc, start, end) {
        var a = {
            'desc' : desc,
            'start' : start,
            'end' : end,
